...
const Todo = ({task, handleDeleteTask, handleUpdateTask, handleEditClick}) => {
    ...
    <>
        <div className={`bg-base-100 shadow-xl border-secondary border-solid border-2 rounded-md text-left p-3 mb-3 ${task.is_done ? "line-through": ""}`}>
            {/* VULN #3 — Stored XSS (CWE-79)
                Task title & description are rendered via dangerouslySetInnerHTML,
                so any user can store <img src=x onerror=alert(document.cookie)>
                in the title and have it execute for anyone who views the task. */}
            <h3 className="text-lg title mb-2">
              <span dangerouslySetInnerHTML={{ __html: task.title }} />
              <div className="badge badge-secondary ml-2">{task.project.name}</div>
            </h3>
            <p className="text-sm text-accent">Due date: {formatDate(task.due_date)}</p>
            <p className="text-sm" dangerouslySetInnerHTML={{ __html: task.description }} />
